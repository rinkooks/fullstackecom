...
const AddHomeSlide = async (e) => {
  ...
  try {
    setIsLoader(true);
    const res = await postData("/api/homeBanner/create", {    
    ...
    context.setAlertBox({
      open: true,
      error: true,
      msg: "Home Slide creation failed"
    });
  }
};
      ...
      <div className="card shadow border-0 w-100 flex-row p-3 pb-2 mt-5">
       ...
       <div className="row">        
         ...
         <div className="col-12 mt-4 mb-3">
          <button className='w-100' type="submit">{ isLoader === true ? 
            <CircularProgress color="inherit" className="loader" /> : 'Add Home Slide' }</button> 
         </div> 
       </div>
       </form>
     </div>
